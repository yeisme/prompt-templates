...
> 状态说明：该 change 保留为 `face-mask-front-v1` 与 `clean-background-plate-v1` 的窄兼容切片。（2026-09-05 更新：`face-mask-front-v1` 随旧 `ai-drama-character-assets@1.x` 退役，本 change 范围收敛为背景侧 `clean-background-plate-v1` 与运行隔离要求；见 `official-ai-drama-character-assets-v1-promotion`。）新的模块化人物、物件、场景和预览能力由 `official-ai-drama-modular-visual-assets-v2` 承接；本 change 不扩张为头发、服饰或整套资产包 owner。
