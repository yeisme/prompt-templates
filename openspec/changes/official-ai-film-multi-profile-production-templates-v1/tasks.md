...
- [ ] 1.1 Use Template Registry authoring commands to create the solution, roles, contracts and fixture skeletons
  ...
  - 复核（2026-09-08）：对照 template-registry HEAD `b4d740d` 复验不变——`release.go:43` 锁定与两个 release family 均无变化，`solutionAdd`（`internal/catalogcmd/command.go:1263`）flag 集仍无 dependency-ref 参数；本仓 `releases/` 仍为 0 文件；本仓基线绿（catalog build 连续两次同 digest `sha256:cd2c5c456fff2e320f26f42e1af66e0e62581d5e2de2945025fbdad7b51416c2`、43 solutions、validate success、工作树零漂移；七 role zh-CN `fixture validate` 全 success，case_count 3/2/3/6/2/2/3）；外部门维持：Registry 需新增普通 prompt solution 的 immutable release/dependency-ref CLI。
  - 复核（2026-09-09）：对照 template-registry HEAD `3bdac00`（较 `b4d740d` 前移，新增 promptlint/归档类提交）复验不变——`internal/structuredrelease/release.go:43` 仍以 `requiredGraphKitRoles` 锁定，CLI 仍仅 graph-kit/ui-template 两个 release family（`internal/catalogcmd/command.go:111-120`），`solutionAdd`（`command.go:1263`）flag 集仍无 dependency-ref 参数；本仓 `releases/` 仍 0 文件；本仓基线绿（catalog build 连续两次同 digest `sha256:fe3bf8b3db2ba8223d16714d7c298d30e5bd05eb606876fa229b5fed15504a0f`、46 solutions、validate success、工作树零漂移；七 role zh-CN `fixture validate` 全 success，case_count project-intent 3 / asset-dependency 2 / continuity-review 2 / failure-restructure 3 / scene-package 3 / shot-audio-intent 2 / shot-prompt 6；`openspec validate --all --strict` 21 passed 0 failed；solution 数 43→46 系并行新增 cinematique/visual-exploration/pixel-battle 全仓条目，与本 change 无关）；外部门维持：Registry 需新增普通 prompt solution 的 immutable release/dependency-ref CLI。
  - 复核（2026-09-11）：对照 template-registry HEAD `9456a03`（较 `3bdac00` 前移）复验不变——`internal/structuredrelease/release.go` 仍以 `requiredGraphKitRoles`（manifest/source_adapter/lens/view/validator）锁定，CLI 仍仅 graph-kit/ui-template 两个 release family，`solutionAdd` flag 集仍无 dependency-ref 参数；本仓 `releases/` 仍 0 文件；本仓基线绿（本会话开始时 catalog digest `sha256:fe3bf8b3…`、46 solutions、`openspec validate --all --strict` 22 passed 0 failed、工作树零漂移；七 role zh-CN `fixture validate` 复跑全 success，case_count 3/2/2/3/3/2/6；46→51 solutions 系本会话 `official-3d-model-templates-beta-v1` 新增 5 个 3d beta 条目，与本 change 无关）；外部门维持：Registry 需新增普通 prompt solution 的 immutable release/dependency-ref CLI。
  - 复核（2026-09-12）：对照 template-registry HEAD `9456a03`（与 2026-09-11 复核相同 HEAD，未前移）复验不变——`internal/structuredrelease/release.go:43` 仍以 `requiredGraphKitRoles`（manifest/source_adapter/lens/view/validator）锁定，CLI 仍仅 graph-kit/ui-template 两个 release family，`solutionAdd`（`internal/catalogcmd/command.go`）flag 集仍无 dependency-ref 参数；本仓 `releases/` 仍 0 文件；本仓基线绿（catalog build 连续两次同 digest `sha256:9de636245d2b5943f3ec07633e3201dae5c41573268ccc9de6e7638627ac8932`、51 solutions、catalog validate success、`openspec validate --all --strict` 22 passed 0 failed、工作树零漂移；七 role zh-CN `fixture validate` 全 success，case_count project-intent 3 / asset-dependency 2 / continuity-review 2 / failure-restructure 3 / scene-package 3 / shot-audio-intent 2 / shot-prompt 6）；外部门维持：Registry 需新增普通 prompt solution 的 immutable release/dependency-ref CLI。
  - 复核（2026-09-13）：对照 template-registry HEAD `c6c597b`（较 `9456a03` 前移，仅 daily-closeout chore 提交，无功能变化）复验不变——`internal/structuredrelease/release.go:43` 仍以 `requiredGraphKitRoles`（manifest/source_adapter/lens/view/validator）锁定，CLI 仍仅 graph-kit/ui-template 两个 release family（`internal/catalogcmd/command.go:111-120`），`solutionAdd`（`command.go:1263`）flag 集仍无 dependency-ref 参数；本仓 `releases/` 仍 0 文件；本仓基线绿（catalog build 多次构建同 digest `sha256:d0c238da964df4fe4355a0c4bc692129e3aa4180d73ec38a0f1bbe2c3f02a7e8`、52 solutions、catalog validate success、`openspec validate --all --strict` 22 passed 0 failed、工作树零漂移；七 role zh-CN `fixture validate` 全 success，case_count project-intent 3 / asset-dependency 2 / continuity-review 2 / failure-restructure 3 / scene-package 3 / shot-audio-intent 2 / shot-prompt 6；51→52 solutions 系本仓 `4b26894` 新增 editorial-half-face-cover solution，与本 change 无关）；外部门维持：Registry 需新增普通 prompt solution 的 immutable release/dependency-ref CLI。
  - 复核（2026-09-14）：对照 template-registry HEAD `c6c597b`（与 2026-09-13 复核相同 HEAD，未前移）复验不变——`internal/structuredrelease/release.go:43` 仍以 `requiredGraphKitRoles`（manifest/source_adapter/lens/view/validator）锁定，CLI 仍仅 graph-kit/ui-template 两个 release family，`solutionAdd`（`internal/catalogcmd/command.go:1268`）flag 集仍无 dependency-ref 参数；本仓 `releases/` 仍 0 文件；本仓基线绿（catalog build 连续两次同 digest `sha256:7da67a6e385f8903219e8ce83b9278b2609422536a7152df7e5a2ba5e9d18b10`、53 solutions、catalog validate success、工作树零漂移——全部校验命令前后 `git status --porcelain` 快照一致；52→53 solutions 系并行 discovery lane 未提交的 pop-science-encyclopedia 新条目，与本 change 无关；`openspec validate --all --strict` 23 passed 0 failed，22→23 系该 lane 的 unified-template-discovery spec）；七 role zh-CN `fixture validate` 全 success，case_count project-intent 3 / asset-dependency 2 / continuity-review 2 / failure-restructure 3 / scene-package 3 / shot-audio-intent 2 / shot-prompt 6；外部门维持：Registry 需新增普通 prompt solution 的 immutable release/dependency-ref CLI。
  - 复核（2026-09-15）：对照 template-registry HEAD `7672921`（较 `c6c597b` 前移，仅 daily-closeout chore 提交，无功能变化）复验不变——`internal/structuredrelease/release.go:43` 仍以 `requiredGraphKitRoles`（manifest/source_adapter/lens/view/validator）锁定，CLI 仍仅 graph-kit/ui-template 两个 release family（`internal/catalogcmd/command.go:114-123`），`solutionAdd`（`command.go:1268`）flag 集仍无 dependency-ref 参数；本仓 `releases/` 仍 0 文件；本仓基线绿（catalog build 连续两次同 digest `sha256:7da67a6e385f8903219e8ce83b9278b2609422536a7152df7e5a2ba5e9d18b10`、53 solutions、catalog validate success、`openspec validate --all --strict` 23 passed 0 failed、工作树零漂移——全部校验命令前后 `git status --porcelain` 快照一致；solution 数与 spec 计数与 2026-09-14 相同，昨日并行 lane 的 pop-science-encyclopedia 条目与 unified-template-discovery spec 已由本仓 `c3bf3c6` 收口提交）；七 role zh-CN `fixture validate` 全 success，case_count project-intent 3 / asset-dependency 2 / continuity-review 2 / failure-restructure 3 / scene-package 3 / shot-audio-intent 2 / shot-prompt 6；外部门维持：Registry 需新增普通 prompt solution 的 immutable release/dependency-ref CLI。
...
- [ ] 3.3 Publish an immutable internal exploratory release through Registry workflow
  ...
  - 复核（2026-09-08）：外部门双门复验不变——template-registry HEAD `b4d740d` 仍仅有 graph-kit（`requiredGraphKitRoles` 锁定）与 ui-template 两个 release family，无普通 prompt solution 的结构化 immutable release 命令；维护者 commit/tag 固定与发布授权仍需人工决策；本仓 `releases/` 仍 0 文件；本地可推进面为零，不得宣称 exploratory release 已存在。
  - 复核（2026-09-09）：外部门双门复验不变——template-registry HEAD `3bdac00`（较 `b4d740d` 前移）仍仅有 graph-kit（`requiredGraphKitRoles` 锁定，`internal/structuredrelease/release.go:43`）与 ui-template 两个 release family，无普通 prompt solution 的结构化 immutable release 命令；维护者 commit/tag 固定与发布授权仍需人工决策；本仓 `releases/` 仍 0 文件；本地可推进面为零，不得宣称 exploratory release 已存在。
  - 复核（2026-09-11）：外部门双门复验不变——template-registry HEAD `9456a03` 仍仅有 graph-kit（`requiredGraphKitRoles` 锁定）与 ui-template 两个 release family，无普通 prompt solution 的结构化 immutable release 命令；维护者 commit/tag 固定与发布授权仍需人工决策；本仓 `releases/` 仍 0 文件；本地可推进面为零，不得宣称 exploratory release 已存在。
  - 复核（2026-09-12）：外部门双门复验不变——template-registry HEAD 仍为 `9456a03`（与 2026-09-11 复核相同，未前移），仍仅有 graph-kit（`requiredGraphKitRoles` 锁定）与 ui-template 两个 release family，无普通 prompt solution 的结构化 immutable release 命令；维护者 commit/tag 固定与发布授权仍需人工决策；本仓 `releases/` 仍 0 文件；本地可推进面为零，不得宣称 exploratory release 已存在。
  - 复核（2026-09-13）：外部门双门复验不变——template-registry HEAD `c6c597b`（较 `9456a03` 前移，仅 daily-closeout chore 提交）仍仅有 graph-kit（`requiredGraphKitRoles` 锁定，`internal/structuredrelease/release.go:43`）与 ui-template 两个 release family，无普通 prompt solution 的结构化 immutable release 命令；维护者 commit/tag 固定与发布授权仍需人工决策；本仓 `releases/` 仍 0 文件；本地可推进面为零，不得宣称 exploratory release 已存在。
  - 复核（2026-09-14）：外部门双门复验不变——template-registry HEAD 仍为 `c6c597b`（与 2026-09-13 复核相同，未前移），仍仅有 graph-kit（`requiredGraphKitRoles` 锁定，`internal/structuredrelease/release.go:43`）与 ui-template 两个 release family，无普通 prompt solution 的结构化 immutable release 命令；维护者 commit/tag 固定与发布授权仍需人工决策；本仓 `releases/` 仍 0 文件；本地可推进面为零，不得宣称 exploratory release 已存在。
  - 复核（2026-09-15）：外部门双门复验不变——template-registry HEAD `7672921`（较 `c6c597b` 前移，仅 daily-closeout chore 提交）仍仅有 graph-kit（`requiredGraphKitRoles` 锁定，`internal/structuredrelease/release.go:43`）与 ui-template 两个 release family（`internal/catalogcmd/command.go:114-123`），无普通 prompt solution 的结构化 immutable release 命令；维护者 commit/tag 固定与发布授权仍需人工决策；本仓 `releases/` 仍 0 文件；本地可推进面为零，不得宣称 exploratory release 已存在。
