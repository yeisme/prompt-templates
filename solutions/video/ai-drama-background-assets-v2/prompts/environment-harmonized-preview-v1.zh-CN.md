# 环境整体协调预览 Prompt Bundle（environment-harmonized-preview-v1）
...
`environment-harmonized-preview-v1` 生成**环境整体协调检查预览**：把场景壳与语义物件按空间锚点合成，并按 `{{render_policy_json}}` 中可选的统一光线、材质观感或色彩协调呈现，用于人工评估环境整体协调度。即使协调后的观感最佳，source slot refs、DesignSpec 与 RenderRevision 仍是后续生成的唯一 canonical inputs；本预览不成为新的 scene-shell 或 object source。

## 无人物边界

预览继承场景壳的无人物合同：人物、肢体、人影、倒影、海报/照片/屏幕中的人物、雕像、模特、驾驶员一律不得进入预览合成要求；协调过程也不得为“画面更完整”带入任何人物痕迹，违者输出 blocking finding。
...
- 协调只作用于预览呈现，不得反写为 source 的锚点、状态或 digest；预览自身不宣称 subject frozen、production ready 或 final accepted。
- 任一 source 的 digest 与绑定不匹配、缺失或 stale 时输出 blocking finding，不得静默降级。

## 编译顺序

1. 从 `{{source_refs_json}}` 读取壳体与物件清单，确认锚点归属与视图匹配。
2. 从 `{{reference_bindings_json}}` 读取 inherit/forbid 声明：预览只读取，不扩展继承范围。
3. 协调呈现：按 `{{render_policy_json}}` 的可选统一项（光线/材质观感/色彩）合成整体画面，并逐项说明协调维度与未改动事实（锚点、光源方向、预留区）。
...
至少输出以下禁止项：改写任何 source 的锚点、状态或 digest；把预览当作任何 source slot 的替代 ref 或新 canonical 资产；人物、人影、倒影、海报/屏幕人物、雕像、模特、驾驶员；新增未声明物件或临时结构；宣称 canonical、subject frozen、production ready；文字、Logo、水印、边框；多视图拼图。
...
- `views`：恰好一个 view，`view_key` 形如 `view-<view_id>`，`view_role` 等于 `{{view_id}}`（`adaptive` 时需 `adaptive_view_reason`），含 `instruction` 与 `negative_constraints`，并声明本轮唯一 `change_scope`（通常为 `lighting`）。
...
`qc_checklist` 必须覆盖：source_refs 与输入逐项一致且排序稳定；协调维度与未改动事实逐项声明；锚点、光源方向与预留区未被改写；无人物/人形痕迹混入；`canonical=false` 与 usage_limits 在场；无文字/边框污染。
...
- lineage 缺失、digest 不匹配、预览被要求作为 canonical 资产、宣称生产就绪或人物痕迹混入时，必须输出 blocking finding（稳定失败码至少：`PREVIEW_SOURCE_LINEAGE_MISSING`、`PREVIEW_SOURCE_DIGEST_MISMATCH`、`PREVIEW_CANONICAL_CLAIM_FORBIDDEN`、`PREVIEW_SUBJECT_FREEZE_CLAIM_FORBIDDEN`、`PREVIEW_ENVIRONMENT_HUMAN_TRACE_DETECTED`）。
