# Prompt Templates CI/CD

本项目采用根级 `docs|quick|full|integration|release` 语义，但 workflow、命令和证据由本仓库自己维护。Profile：`content catalog`。
...
- `jq empty catalog.json repository.json`
- `find . -name solution.json -print0 | xargs -0 -r -n1 jq empty`
...
No binary CD; catalog generation remains owned by Template Registry CLI.
